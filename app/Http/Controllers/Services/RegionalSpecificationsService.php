...
class RegionalSpecificationsService extends Controller
{
    public $name = "المواصفات الإقليمية";
    public $title_creator = "إضافة مواصفات إقليمية جديدة";
    public $title_updater = "تعديل بيانات المواصفات الإقليمية";
    public $modal_size = "";
    public $creator_id = "creator-feature-button";
    public $updater_id = "updater-feature-button";
    public $model = RegionalSpecification::class;

    public function __construct()
    {
        $this->model = new RegionalSpecification();
    }

    public function model($id)
    {
        return RegionalSpecification::find($id);
    }

    public function data($filters, $sort_field, $sort_direction, $paginate = 10)
    {
        return RegionalSpecification::data()
            ->filters($filters)
    ...
    {
        return RegionalSpecification::getRules($id);
    }

    public function messages($id = "")
    {
        return RegionalSpecification::getMessages($id);
    }


    public function delete($id)
    {
        return RegionalSpecification::deleteModel($id);
    }

    public function status($id)
    {
        return RegionalSpecification::status($id);
    }

    public function store($data)
    {
        return RegionalSpecification::store($data);
    }

    public function update($data, $id)
    {
        return RegionalSpecification::updateModel($data, $id);
    }
